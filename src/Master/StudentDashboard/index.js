...
const mapStateToProps = (store) => {
	return {
		...
		isApiFetchError: store.student.isApiFetchError,
	};
};
...
class StudentDashboard extends React.Component {
	...
	componentWillReceiveProps(nextProps) {
		...
		if (nextProps.studentData && nextProps.studentData.length !== 0) {
			...
			this.setState({
				isApiError: nextProps.isApiFetchError.isFetchError,
			});
		}
	}
		...
		this.setState(
			...
			() => {
				var res = Object.keys(this.state.studentData)
					.filter(
						(keys) =>
							this.state.studentData[keys].name.toLowerCase().indexOf(input) > -1
					)
					.reduce(
						(res, key) => Object.assign(res, { [key]: this.state.studentData[key] }),
						{}
					);
	...
	showErrorScreen = () => {
		...
		return <div className="api-error">There is some issue with the API.</div>;
	};

	render() {
		console.log("In  render", this.state.isApiError);
		return (
			<div className="dashboard-main-container">
				{this.state.isLoader && !this.state.isApiError ? (
					this.showLoader()
									...
									{Object.keys(this.state.studentData).map((value, key) => {
										return (
											...
											>
												...
												>
													<div>
														Id: {this.state.studentData[value].rollNo}
													</div>
													<div>
														Name: {this.state.studentData[value].name}
													</div>
													<div>
														Total Marks:{" "}
														{this.getTotalMarks(
